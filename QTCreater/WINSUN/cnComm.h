...
#define COMMPROP_INITIALIZED ((DWORD)0xE73CF52E) //CYP
...
public:
	...
	{
//		if(!CN_ASSERT(_tcslen(szPortName) < 64 - 1))//cyp
        if(!CN_ASSERT(lstrlen(szPortName) < 64 - 1))
			return false;
		
//		_tcscpy(szName_, szPortName);//cyp
        lstrcpy(szName_, szPortName);
