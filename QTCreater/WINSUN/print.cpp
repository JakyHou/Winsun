...
//    QMessageBox::information(0,"打印停止!","打印已停止\n\n"+m_sAbortMessage);
    ...
    if(language=="Chinese"){
        QMessageBox::information(0,"打印终止!","打印终止\n\n"+m_sAbortMessage);
    }else{
...
void Print::exposeTBaseLayer(){
    ...
    if(language=="Chinese"){
        ui->lineEditLayerCount->setText("正在打印切片 "+QString::number(m_iCurLayerNumber+1)+" of "+QString::number(m_iLastLayer)+",  "+QString::number(100.0*(double)(m_iCurLayerNumber+1)/(double)m_iLastLayer,'f',1)+"% 完成");
    }else{
...
//        m_pTerminal->rcCloseVat();
        if(language=="Chinese"){
            ui->pushButtonPauseResume->setText("恢复");//继续
            ui->pushButtonPauseResume->setToolTip("恢复");
            ui->lineEditLayerCount->setText("已暂停.  手动定位切换开关已启用.");
        }else{
...
//        ui->lineEditLayerCount->setText("Paused.  Manual positioning toggle switches are enabled.");
    ...
    if(m_bAbort){
        ...
        m_pTerminal->rcSetCPJ(NULL); //blank
        if(language=="Chinese"){
            ui->pushButtonAbort->setText("终止");//终止
        }else{
...
//        ui->pushButtonAbort->setText("Abort");//终止
        ...
        if(language=="Chinese"){
            ui->lineEditLayerCount->setText("完成!");//完成
        }else{
...
//        setProjMessage("(按'p' 暂停, 'a' 终止)  " + sTimeUpdate+"  释放并循环到 "+QString::number(m_iCurLayerNumber+1)+"/"+QString::number(m_iLastLayer));
        if(language=="Chinese"){
            ui->lineEditLayerCount->setText("释放层 "+QString::number(m_iCurLayerNumber)+", 重新定位层 "+QString::number(m_iCurLayerNumber+1));
        }else{
...
//    ui->pushButtonPauseResume->setEnabled(false);
    ...
    if(m_iPaused==PAUSE_YES){
        ...
        m_iPaused = PAUSE_NO;
        if(language=="Chinese"){
            ui->pushButtonPauseResume->setText("暂停");
        }else{
...
//        ui->pushButtonAbort->setText("Abort");
    ...
    if(language=="Chinese"){
        ui->pushButtonAbort->setText("终止...");//终止
        ui->lineEditLayerCount->setText("终止...");
    }else{
...
void Print::UpdateByLanguage(){
    if(language=="Chinese"){
        this->setWindowTitle("打印中");
        ui->label->setText("剩余时间:");
        ui->label_2->setText("完成时间：");
        ui->pushButtonPauseResume->setText("暂停");
        ui->pushButtonAbort->setText("终止");

    }else{
        this->setWindowTitle("Printing");
        ui->label->setText("Remaining Time：");
        ui->label_2->setText("Completion Time：");
        ui->pushButtonPauseResume->setText("Pause");
        ui->pushButtonAbort->setText("Abort");
    }
}
